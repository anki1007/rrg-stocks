...
def get_available_csv_files(ticker_folder="tickers"):
    ...
    try:
        ...
        st.warning(f"Could not read tickers folder: {e}")
    return []
...
def main():
    ...
    with st.sidebar:
        ...
        if available_csvs:
            ...
            st.error("⚠️ No CSV files found in 'tickers' folder")
            tickers = []
    ...
    elif not run_screener:
        # Initial instruction screen
        st.info(
            """
            ### 🚀 Getting Started
            
            1. **Select Index** from the dropdown in the left sidebar
            
            2. **Adjust Filters** to customize your screening criteria
            
            3. **Click "Run Screener"** to identify top-performing stocks
            
            4. **Analyze Results** using multiple views (Rankings, Charts, Details)
