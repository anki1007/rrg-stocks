...
st.set_page_config(
    layout="wide",
    page_title="RRG Dashboard - Nifty Sector Indices",
...
# CSV path for Nifty Indices
NIFTY_INDICES_CSV = "https://raw.githubusercontent.com/anki1007/rrg-stocks/main/ticker/niftyindices.csv"
...
def load_nifty_indices():
    """Load Nifty sector indices from GitHub CSV"""
    try:
        df = pd.read_csv(NIFTY_INDICES_CSV)
        return df
    except Exception as e:
        st.error(f"Failed to load indices: {str(e)}")
        return pd.DataFrame()
...
if st.session_state.load_clicked:
    try:
        ...
        # Load Nifty sector indices from CSV
        universe = load_nifty_indices()
        if universe.empty:
            st.error("❌ Failed to load Nifty indices data from GitHub.")
            st.stop()
        ...
        for s in symbols:
            ...
            try:
                ...
                tail_length = min(14, len(rs_ratio))
...
if st.session_state.df_cache is not None:
    ...
    st.markdown("# 📊 Relative Rotation Graph - Nifty Sector Indices")
    st.markdown(f"**Nifty Sector Indices | {tf_name} | {period_name} | Benchmark: {bench_name}**")
    ...
    # ========================================================================
    with col_main:
        df_graph = select_graph_stocks(df, min_stocks=min(40, len(df)))
                    ...
                    if row['Symbol'] in rs_history:
                        ...
                        if len(rs_ratio_tail) >= 2:
                            ...
                            fig_rrg.add_annotation(
                                ...
                                arrowwidth=2.5, arrowcolor=QUADRANT_COLORS[status]
                            )
    ...
    with col_right:
        ...
        top_df = df.nlargest(top_n, 'RRG Power')
        for idx, row in top_df.iterrows():
            status_emoji = {"Leading": "🟢", "Improving": "🔵", "Weakening": "🟡", "Lagging": "🔴"}
            st.markdown(f"**{row['Rank']}. {row['Symbol']}** {status_emoji[row['Status']]}")
            st.markdown(f"Power: {row['RRG Power']:.2f}")
            st.markdown("---")
    ...
    display_df = df[['Sl No.', 'Symbol', 'Name', 'Industry', 'Price', 'Change %', 
                     'RS-Ratio', 'RS-Momentum', 'RRG Power', 'Distance', 
                     'Direction', 'Velocity', 'Status']].copy()
    ...
    if export_csv:
        ...
        st.download_button(
            ...
            data=csv,
            file_name=f"nifty_rrg_{tf_name}_{period_name}_{datetime.now().strftime('%Y%m%d')}.csv",
            mime="text/csv",
        )
    ...
    st.markdown("---")
    st.markdown("### About RRG Analysis")
    st.markdown("""
    - **Leading** (🟢): Strong relative strength, positive momentum
    - **Improving** (🔵): Weak relative strength, positive momentum  
    - **Weakening** (🟡): Weak relative strength, negative momentum
    - **Lagging** (🔴): Strong relative strength, negative momentum

    **Data Source**: GitHub CSV - Nifty Sector Indices
    """)
