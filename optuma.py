...
def calculate_jdk_rrg(ticker_series, benchmark_series, window=WINDOW):
    ...
    # Higher distance = stronger signal, lower = closer to benchmark
    distance = np.sqrt((rs_ratio - 100) ** 2 + (rs_momentum - 100) ** 2)
    
    # 6. Calculate JDKRS().Heading - Direction in degrees (0-360)
    # 0° = East, 90° = North, 180° = West, 270° = South
    # Northeast (22.5-67.5°) = Leading direction (strong RS, increasing momentum)
    ...
    # Distance between last two points (rate of change)
...
# ============================================================================
# SIDEBAR - CONTROLS WITH DEFAULTS
# ============================================================================
...
with col_main:
    ...
    st.markdown(f"**{csv_selected} | {tf_name} | {period_name} | Benchmark: {bench_name} ({BENCHMARKS[bench_name]})**")
        ...
        if not df_status.empty:
            ...
            for _, row in df_status.iterrows():
                ...
                hover_info = (
                    ...
                    f"Distance: {row['Distance']:.2f}<br>"
                    f"Heading: {row['Heading']:.1f}° {row['Direction']}<br>"
                    f"Velocity: {row['Velocity']:.3f}<br>"
    ...
    fig_rrg.update_layout(
        height=500,
        xaxis_title="RS-Ratio (JdK X-axis)",
        yaxis_title="RS-Momentum (JdK Y-axis)",
