...
if not os.path.exists(CFG_PATH):
    DEFAULT_CFG = {
        ...
        "cache_dir": "cache",
        # your GitHub ticker folder (RAW URLs)
...
# Merge config tickers with REQUIRED_TICKERS
    ...
    for fname in files:
        try:
            ...
            if cols:
                ...
                # assume NSE equities unless prefixed/indices
                for v in vals:
                    v = v.strip()
...
def display_name(sym: str) -> str:
    return safe_long_name_cached(sym)  # same logic
...
# Universe (required + GitHub lists)
...
# Visible selection (mirrors checkbox column)
...
# --- Colored HTML table with TradingView links (Name column) ---
...
.rrg-table th, .rrg-table td { padding:6px 8px; border:1px solid rgba(128,128,128,0.25); font-family: Segoe UI, Arial, system-ui; font-size:13px; }
.rrg-table th { position:sticky; top:0; background:rgba(200,200,200,0.10); text-align:left; }
.rrg-color { width:14px; height:14px; border-radius:2px; display:inline-block; border:1px solid rgba(0,0,0,0.25); }
</style>
...
for sym, name, status, price, chg in rows:
    ...
    bg = status_bg_color(float(rs_ratio_map[sym].iloc[sel_pos]), float(rs_mom_map[sym].iloc[sel_pos]))
    ...
    html.append(
        ...
        f'<td><a href="{url}" target="_blank" rel="noopener noreferrer">{name}</a></td>'
        f'<td>{status}</td>'
...
with colB:
    if st.button("Export Table CSV"):
        df = pd.DataFrame(
            [{
                "SL No.": rank_map.get(sym,""),
                " ": COLORS.get(sym, "#888"),
                "Name": display_name(sym),
                "Status": get_status(float(rs_ratio_map[sym].iloc[sel_pos]), float(rs_mom_map[sym].iloc[sel_pos])),
                "Price": fmt_num(float(px_cache.get(sym, pd.Series(dtype=float)).iloc[sel_pos]) if sel_pos < len(px_cache.get(sym, pd.Series(dtype=float))) else np.nan,2),
                "Change %": fmt_num(((px_cache[sym].iloc[sel_pos]/px_cache[sym].iloc[start_pos]-1)*100.0) if (sym in px_cache and sel_pos < len(px_cache[sym]) and start_pos < len(px_cache[sym])) else np.nan,2)
            } for sym,_,_,_,_ in rows if sym in visible]
        )
...
with colC:
    if st.button("Export Chart PNG"):
        ...
        st.download_button("Download rrg.png", buf.getvalue(),
